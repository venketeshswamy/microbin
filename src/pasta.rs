...
pub struct Pasta {
    ...
    /// Optional user-defined slug. When set, the paste is accessible via
    /// /upload/{custom_slug} in addition to /upload/{id_as_animals}.
    #[serde(default)]
...
impl Pasta {
    pub fn find_index(pastas: &[Pasta], slug_or_id: &str) -> Option<usize> {
        let numeric_id = if ARGS.hash_ids {
            hashid_to_u64(slug_or_id).unwrap_or(0)
        } else {
            to_u64(slug_or_id).unwrap_or(0)
        };
        
        // Try to match by numeric ID first
        if numeric_id != 0 {
            if let Some(pos) = pastas.iter().position(|p| p.id == numeric_id) {
                return Some(pos);
            }
        }

        // Try custom slug next
        pastas.iter().position(|p| p.custom_slug.as_deref() == Some(slug_or_id))
    }
