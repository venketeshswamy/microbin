...
pub async fn create(
    ...
    let mut custom_slug: String = String::from("");
    ...
    while let Some(mut field) = payload.try_next().await? {
        ...
        match field_name {
            ...
            "custom_slug" => {
                while let Some(chunk) = field.try_next().await? {
                    custom_slug = std::str::from_utf8(&chunk)
                        .unwrap_or("")
                        .trim()
                        .to_string();
                }
    ...
    // Feature 2: Custom URL / Slug — store the slug directly on the pasta.
    // The pasta ID stays randomly generated; the slug is used as an alias for lookup.
    if !custom_slug.is_empty() {
        let pastas = data.pastas.lock().unwrap();
        let slug_taken = pastas.iter().any(|p| {
            p.custom_slug.as_deref() == Some(custom_slug.as_str())
                || p.id_as_animals() == custom_slug
        });
        if slug_taken {
            log::warn!("Custom slug '{}' is already taken", custom_slug);
            return Err(ErrorBadRequest("Custom URL slug is already taken. Please choose another one."));
        } else {
            log::info!(
                "Custom slug '{}' accepted for pasta ID {}",
                custom_slug, new_pasta.id
            );
            new_pasta.custom_slug = Some(custom_slug.clone());
        }
    ...
    // Capture custom_slug before new_pasta is moved into the pastas vec
    let custom_slug_for_redirect = new_pasta.custom_slug.clone();
    ...
    // Use custom slug in the URL if one was accepted, so the browser lands on /upload/my-note
    let slug = if let Some(cs) = custom_slug_for_redirect {
        cs
