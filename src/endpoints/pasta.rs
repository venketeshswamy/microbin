...
fn pastaresponse(
    ...
    // Save the raw slug string before consuming the Path (needed for custom slug fallback)
...
pub async fn getpasta(
    ...
    if let Some(cookie) = req.cookie("owner_token") {
        ...
        if verify_owner_token(cookie.value(), &id, &pastas) {
...
fn verify_owner_token(token: &str, id: &str, pastas: &[Pasta]) -> bool {
    // decode the token
    if let Ok(numbers) = crate::util::hashids::HARSH.decode(token) {
        if numbers.len() == 2 {
            ...
            let id_matched = if token_id == target_id {
                true
            } else {
                pastas.iter().any(|p| p.id == token_id && p.custom_slug.as_deref() == Some(id))
            };

            if id_matched && expiry > timenow {
                // yay, it's valid
                return true;
            }
        }
